...
<body>
    ...
    <div class="website-modals">
        ...
        <div id="website-modal-{{$modal->id}}" class="modal"><div class="modal-background"></div><div class="modal-card"><header class="modal-card-head has-text-centered"><p class="modal-card-title">{{$modal->name}}</p><button class="delete" aria-label="close" onclick="toggleIsActive(this.parentElement.parentElement.parentElement)"></button></header><section class="modal-card-body"><!-- Content Start --><div class="columns is-centered"><div class="column is-11"><figure class="image is-4by3"><img src="https://bulma.io/images/placeholders/1280x960.png" alt="Placeholder image"/></figure><p class="is-size-5 mt-3 mb-2">{{$modal->description}}</p><hr style="width: 100%;" /><span class="is-size-3">Features</span><ul class="pl-2"><li>&mdash; a</li><li>&mdash; s</li><li>&mdash; d</li><li>&mdash; d</li><li>&mdash; sd</li></ul></div></div><!-- Content End --></section><footer class="modal-card-foot"><button class="button is-primary"> <a href="{{$modal->link}}" target="_blank" style="color:#FFF">View Demonstration</a></button><button class="button" onclick="toggleIsActive(this.parentElement.parentElement.parentElement)">Close</button></footer></div></div>
        @endforeach
    </div>
</body>
</html>
